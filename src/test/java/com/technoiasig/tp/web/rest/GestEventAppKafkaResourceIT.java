...
import com.technoiasig.tp.config.KafkaProperties;
...
class GestEventAppKafkaResourceIT {
    ...
    void setup() {
        ...
        GestEventAppKafkaResource kafkaResource = new GestEventAppKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/gest-event-app-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/gest-event-app-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
